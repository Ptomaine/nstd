...
namespace agg
{
    ...
    font_engine_win32_tt_base::font_engine_win32_tt_base(bool flag32, 
        ...
        m_fonts1024(new HFONT [max_fonts]),
        m_num_fonts(0),
                                                ...
                                                glyph_rendering ren_type)
    {
        if(m_dc)
        {
            ...
            if(m_resolution)
            {
                h = ::MulDiv(int(m_height), m_resolution, 72);
                w = ::MulDiv(int(m_width),  m_resolution, 72);
            }
            ...
            if(idx >= 0)
            {
                ...
                m_cur_font1024 = m_fonts1024[idx];
                m_num_kerning_pairs = 0;
                return true;
            }
            else
            {
                m_cur_font = ::CreateFont(
                   -h,                      // height of font
                ...
                if(m_cur_font && m_cur_font1024)
                {
                    ...
                    m_fonts1024[m_num_fonts] = m_cur_font1024;
                    ++m_num_fonts;
                    m_num_kerning_pairs = 0;
                    return true;
                }
    ...
    void font_engine_win32_tt_base::update_signature()
    {
        ...
        if(m_dc && m_cur_font)
        {
            ...
            sprintf(m_signature, 
                    "%s,%u,%d,%d:%dx%d,%d,%d,%d,%d,%d,%08X,%d,%d,%d,%d", 
                    m_typeface,
                    m_char_set,
...
#define GGO_UNHINTED 0x0100
            ...
            ::SelectObject(m_dc, m_cur_font1024);
            int total_size = GetGlyphOutlineX(m_dc,
                                              glyph_code,
                                              GGO_METRICS | GGO_UNHINTED,
                                              &gm,
                                              0,
                                              0,
                                              &identity);
            ...
            m_advance_x =  gm.gmCellIncX * 
                           m_height * 
                           (m_matrix.eM11.value + m_matrix.eM11.fract / 65536.0) / 
                           1024.0;


            m_advance_y = -gm.gmCellIncY * m_height / 1024.0;

            ::SelectObject(m_dc, m_cur_font);
            total_size = GetGlyphOutlineX(m_dc,
                                          glyph_code,
                                          format,
                                          &gm,
                                          buf_size,
                                          ...
                                          &m_matrix);
//ABCFLOAT abc;
//::GetCharABCWidthsFloat(m_dc, glyph_code, glyph_code, &abc);
//m_advance_x = (abc.abcfA + abc.abcfB + abc.abcfC) * (m_matrix.eM11.value + m_matrix.eM11.fract / 65536.0);
