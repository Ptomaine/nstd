 ...
 * ...
 * void open_handler(const asio::error_code& ec)
 * {
 *   if (!ec)
 *   {
...
public:
   ...
   */
  explicit read_stream(asio::io_context& io_context)
    : io_context_(io_context),
      file_(io_context, options_),
      http_(io_context, options_),
#if !defined(URDL_DISABLE_SSL)
      ssl_context_(io_context, asio::ssl::context::sslv23),
   ...
   * with arrays, @c std::array or @c std::vector.
   */
